...
class TestZeroMQSubSource(unittest.TestCase):
    ...
    test_pub = ZeroMQPubSink(endpoint, topic)
    ...
    def test_socket_connect_and_close_success(self):
        zeromq = ZeroMQSubSource(endpoint=TestZeroMQSubSource.endpoint, socket_type=TestZeroMQSubSource.topic)
    ...
    def test_message_receive_success(self):
        zeromq = ZeroMQSubSource(endpoint=TestZeroMQSubSource.endpoint, topic=TestZeroMQSubSource.topic)
        self.save_connect(zeromq)
        time.sleep(1)
