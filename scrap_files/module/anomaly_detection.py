...
from analytics_core.modules.anomaly_detection.log_anomaly_detection import LogAnomalyDetector, log_anomaly_detection
...
class AnomalyDetectionModule(Module, Context, AbstractHandler):
    ...
    def flush(self, context: Optional[LogsightLog] = None) -> Optional[LogsightLog]:
    ...
    def transform(self, data: LogsightLog) -> Optional[LogsightLog]:
    ...
    def _handle(self, context: Optional[LogsightLog]) -> Optional[LogsightLog]:
        result = None
        try:
            result = self.transform(context)
...
class IdleState(State):
    def flush(self, **kwargs) -> Optional[LogsightLog]:
        return
    ...
    def handle(self, request: Optional[LogsightLog] = None) -> Optional[LogsightLog]:
        try:
